...
const Item = () => {
  ...
  async function putForSale(id, price) {
    try {
      ...
      // await NFT_TokenContract.methods.approve(marketAddress, items[itemIdex].tokenId).send({from: accounts[0]});

      const receipt = await marketplaceContract.methods
        .putItemForSale(id, Web3.utils.fromWei(String(price), "ether"))
        .send({ gas: 210000, value:  Web3.utils.fromWei(String(price), "ether"), from: account });
                ...
                <fieldset>
                  ...
                  <div className={classes.buttons}>
                    {owner === account && !isForSale && (
                      <Button
                        ...
                        variant="contained"
                        color="primary"
                        onClick={() => putForSale(tokenId, Web3.utils.fromWei(String(price), "ether"))}
                      >
                        Sell
                      </Button>
                    )}
                  ...
                  {owner !== account && isForSale && (
                    <Button
                      ...
                      onClick={() => buy(saleId, Web3.utils.fromWei(String(price), "ether"))}
                    >
                      Buy
                    </Button>
                  )}
