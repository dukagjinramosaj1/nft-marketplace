...
const CreateNFT = () => {
  ...
  const NFT_TokenContract = useSelector(
    (state) => state.allNft.NFT_TokenContract
  );
  ...
  async function createNFT(event) {
    ...
    try {
      const totalSupply = await NFT_TokenContract.methods.totalSupply().call();
      ...
      console.log("RESPONSE FOR CREATING NFT IS",response.data.message);
  ...
  async function mint(tokenMetadataURL) {
    try {
      const receipt = await NFT_TokenContract.methods
        .mint(tokenMetadataURL)
        .send({ from: account });
      console.log("RECEIPT",receipt);
      console.log("EVENT LOG IS:",receipt.events.Transfer.returnValues.tokenId);
